...
import { FiPlay, FiPause, FiEdit2, FiPlus, FiSearch, FiX, FiWifi, FiRefreshCw, FiChevronRight, FiUsers } from 'react-icons/fi';

const statusLabels: Record<string, string> = {
  ACTIVE: 'Ativa', ENABLED: 'Ativa',
  PAUSED: 'Pausada',
  DRAFT: 'Rascunho',
  ERROR: 'Erro',
  DELETED: 'Excluída',
...
const statusClass: Record<string, string> = {
  ACTIVE: 'badge-active', ENABLED: 'badge-active',
  PAUSED: 'badge-paused',
  DRAFT: 'badge-draft',
  ERROR: 'badge-error',
  DELETED: 'badge-draft',
...
export default function CampaignsPage() {
  ...
  const { campaigns: clientCampaigns, loading: campaignsLoading, refetch: refetchCampaigns } = useClientCampaigns({
  ...
  const toggleStatus = async (id: string, platform: string) => {
    ...
    const newStatus = (campaignToToggle.status === 'ACTIVE' || campaignToToggle.status === 'ENABLED') ? 'PAUSED' : 'ACTIVE';

    // Optimistic UI update
    setCampaigns(prev => prev.map(c => c.id !== id ? c : { ...c, status: newStatus }));
    ...
    if (platform === 'meta') {
      try {
        ...
        if (!data.success) {
          ...
          setCampaigns(prev => prev.map(c => c.id !== id ? c : { ...c, status: campaignToToggle.status }));
        }
      } catch {
        setCampaigns(prev => prev.map(c => c.id !== id ? c : { ...c, status: campaignToToggle.status }));
      }
      ...
      try {
        const account = selectedClient?.accounts.find(a => a.platform === 'google');
        if (!account) return;
        const res = await fetch('/api/google/campaigns/mutate', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            clientAccountId: account.id,
            operations: [{ type: 'UPDATE_STATUS', campaignId: id, status: newStatus }],
          }),
        });
        const data = await res.json();
        if (!data.success) {
          alert(`Erro ao alterar status no Google: ${data.error}`);
          setCampaigns(prev => prev.map(c => c.id !== id ? c : { ...c, status: campaignToToggle.status }));
        }
      } catch {
        setCampaigns(prev => prev.map(c => c.id !== id ? c : { ...c, status: campaignToToggle.status }));
  ...
  return (
    <>
      <Header title="Campanhas" subtitle={selectedClient?.name || 'Gerencie suas campanhas'} />
      <div className="page-content">
        ...
        {!selectedClient && (
          <div style={{
            ...
            <p style={{ color: 'var(--text-secondary)' }}>Selecione um cliente no menu lateral para ver as campanhas</p>
          </div>
        )}
                    ...
                    <tr key={c.id}>
                      <td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <FiWifi size={12} color="var(--accent-primary)" title="Dados Reais" />
                          <div>
                            <div
                      ...
                      <td>R$ {(c.budget || 0).toLocaleString('pt-BR')}</td>
                      <td>R$ {(c.spend || 0).toLocaleString('pt-BR')}</td>
                      <td>{(c.impressions || 0).toLocaleString('pt-BR')}</td>
                      <td>{(c.ctr || 0).toFixed(2)}%</td>
                      <td>R$ {(c.cpc || 0).toFixed(2)}</td>
                      <td>{(c.conversions || 0).toLocaleString('pt-BR')}</td>
                      ...
                      <td>
                        <div style={{ display: 'flex', gap: 4 }}>
                          <button
                            className={`btn btn-sm ${(c.status === 'ACTIVE' || c.status === 'ENABLED') ? 'btn-danger' : 'btn-success'}`}
                            onClick={() => toggleStatus(c.id, c.platform)}
                            title={(c.status === 'ACTIVE' || c.status === 'ENABLED') ? 'Pausar' : 'Ativar'}
                          >
                            {(c.status === 'ACTIVE' || c.status === 'ENABLED') ? <FiPause size={14} /> : <FiPlay size={14} />}
                          </button>
                        </div>
                      </td>
                    </tr>
              ...
              {(selectedAdSet ? realAds : selectedCampaign ? realAdSets : filtered).map((c: any) => (
                <div key={c.id} className="mobile-campaign-card">
                  ...
                  <div className="mobile-campaign-card-metrics">
                    <div className="mobile-metric"><div className="mobile-metric-label">Gasto</div><div className="mobile-metric-value">R$ {(c.spend || 0).toLocaleString('pt-BR')}</div></div>
                    <div className="mobile-metric"><div className="mobile-metric-label">CTR</div><div className="mobile-metric-value">{(c.ctr || 0).toFixed(2)}%</div></div>
                    <div className="mobile-metric"><div className="mobile-metric-label">CPC</div><div className="mobile-metric-value">R$ {(c.cpc || 0).toFixed(2)}</div></div>
                    <div className="mobile-metric"><div className="mobile-metric-label">ROAS</div><div className="mobile-metric-value" style={{ color: c.roas >= 3 ? 'var(--success)' : 'var(--warning)' }}>{c.roas > 0 ? `${c.roas.toFixed(1)}x` : '—'}</div></div>
                  </div>
                  <div className="mobile-campaign-card-actions">
                    <button
                      ...
                      onClick={() => toggleStatus(c.id, c.platform)}
                    >
                      {(c.status === 'ACTIVE' || c.status === 'ENABLED') ? <><FiPause size={12} /> Pausar</> : <><FiPlay size={12} /> Ativar</>}
                    </button>
                  </div>
                </div>
              ))}
                  ...
                  <div className="empty-state">
                    ...
                    <p className="empty-state-text">
                      {selectedClient.accounts.length === 0
                        ...
                        : selectedAdSet ? 'Nenhum anúncio encontrado' : selectedCampaign ? 'Nenhum conjunto de anúncios' : 'Nenhuma campanha encontrada'}
                    </p>
                  </div>
                );
              }
              ...
              <div className="modal-overlay" onClick={() => setShowCreateModal(false)}>
                <div className="modal" onClick={e => e.stopPropagation()}>
                  <div className="modal-header">
                    ...
                    <button className="btn btn-secondary btn-icon" onClick={() => setShowCreateModal(false)}><FiX /></button>
                  </div>
